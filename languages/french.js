...
<:void_bots:738451886147108925> [Voter sur VoidBots](https://voidbots.net/bot/628186022991233025) (**${vb}** votes - Votez toutes les 12H)`,
      ...
      ANIME_VOICE: 'Voix (VO):',
      ANIME_STAFF_ROLE: (staffRole) => `${staffRole === 'Assistant Producer' ? 'Producteur assistant' : staffRole === 'Director, Character Design' ? 'Directeur, Desing des personnages' : staffRole === 'Director, Episode Director' ? "Directeur, Directeur d'épisode" : staffRole === 'Episode Director, Storyboard' ? "Directeur d'épisode, Storyboard" : staffRole === 'Producer' ? 'Producteur' : staffRole === 'Director' ? 'Directeur' : staffRole === 'Sound Director' ? 'Directeur son' : staffRole === 'Script, Screenplay' ? 'Script, Scénario' : staffRole}`,
...
A : ${user.counts.A} - S : ${user.counts.S} - SH : ${user.counts.SH} - SS : ${user.counts.SS} - SSH :${user.counts.SSH}`,
      ...
      ROLE_INFO_EXAMPLES: (prefix) => `${prefix}role-info @Membres\n${prefix}role-info 699011821654507572`,
...
**__We are sorry for the lack of updates, our developers don't have the time they would like to have to take care of Lycos in the best possible way__**`,
      ...
      NOWPLAYING_REQUESTED_BY: 'Demandée par',
      /* Clearqueue */
      CLEARQUEUE_DESCRIPTION: "Supprime les musiques dans la file d'attente",
      ...
      /* Loop */
      LOOP_DESCRIPTION: 'Joue en boucle la queue actuelle.',
      LOOP_USAGE: (prefix) => `${prefix}loop`,
      LOOP_EXAMPLES: (prefix) => `${prefix}loop`,
      LOOP_UNLOOPING: 'Mode boucle déactivé ! La queue actuelle ne sera plus jouée encore et encore...',
      LOOP_LOOPING: "Mode boucle activé ! La queue actuelle sera jouée encore et encore jusqu'à ce que vous fassiez à nouveau la commande !",
      /* Repeat */
      REPEAT_DESCRIPTION: 'Joue en boucle la musique actuelle.',
      REPEAT_USAGE: (prefix) => `${prefix}repeat`,
      REPEAT_EXAMPLES: (prefix) => `${prefix}repeat`,
      REPEAT_UNLOOPING: 'Mode boucle déactivé ! La musique actuelle ne sera plus jouée encore et encore...',
      REPEAT_LOOPING: "Mode boucle activé ! La musique actuelle sera jouée encore et encore jusqu'à ce que vous fassiez à nouveau la commande !",
      /* Pause */
      ...
      REMOVE_ZERO: 'Vous ne pouvez pas retirer la musique en cours de lecture !',
...
**Localisation du serveur :** ${r}`,
      ...
      LOGS_GUILD_MEMBER_CHUNK_TITLE: "Tout un régiment de membres vient d'arriver d'un même serveur !",
