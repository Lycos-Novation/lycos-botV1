...
**Précipitations :** ${result[0].forecast[4].precip}%`,
			...
			SETUP_DESCRIPTION: "Permet de configurer le rolereaction",
			SETUP_USAGE: (prefix) => `${prefix}setup [launch/add/remove] [emote] [Role]`,
			SETUP_EXAMPLE: (prefix) => `${prefix}setup add :lycos: LycosRole`,
			SETUP_SUPPLY_METHOD: "Veuillez préciser ce que vous voulez faire. Répondez avec launch, add ou remove",
			SETUP_EMPTY: "Le role reaction est vide, veuillez ajouter des roles pour pouvoir l'initialiser.",
			SETUP_SUPPLY_EMOTE: "Veuillez indiquer l'emote à ajouter",
			SETUP_SUPPLY_NAME: "Veuillez indiquer le rôle à associer à l'emote",
			SETUP_ALREADY_IN: "Ce rôle est déjà dans le role reaction",
			SETUP_LIMIT: "Vous avez atteint la limite de 30 rôles dans le role reaction.",
			SETUP_NOT_IN: "Ce rôle n'est pas dans le role reaction.",
			SETUP_BAD_METHOD: "Je n'ai pas compris ce que vous vouliez faire. Veuillez recommencer.",
			SETUP_ROLE_ADDED: "Rôle ajouté dans le role reaction.",
			SETUP_NO_CHANNEL: "Veuillez paramétrer un salon pour le rolereaction au préalable. (Voir commmande setnotif)",
			SETUP_SUPPLY_DESCRIPTION: "Veuillez fourir une courte description du rôle. Vous ne pouvez pas utilier le caractère `/` dans votre description.",
			SETUP_ERROR_DESC: "L'utilisation du caratère `/` est réservée à un usage système. Veuillez recommencer.",
			SETUP_EMBED_FOOTER: "Cliquez sur les réactions ci-dessous",
			SETUP_EMBED_TITLE: "Rôle Réaction",
			SETUP_EMBED_DESC: "Cliquez sur la réaction correspondant au rôle que vous souhaitez avoir",
			SETUP_EMBED_FIELD: "Liste des rôles :",
