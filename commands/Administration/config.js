...
const twitchClient = TwitchClient.withClientCredentials(this.client.config.twitch.clientID, this.client.config.twitch.secret)
...
class Config extends Command {
  ...
  async run (message, args) {
    try {
      let g
      mysqlcon.query(`SELECT * FROM Guilds WHERE guild_id='${message.guild.id}'`, async function (err, result, fields) {
        if (err) throw err
        g = result[0]
        const ar = result[0].autorole
        const stream_ar = g.streamers_ids
        var autorole = ''
